...
READING_INTERVAL = 3  # Intervalo em segundos entre as leituras da IA
    ...
    while True:
        if not queue.empty():
            ...
            if processed_image_path is not None:
                ...
                elif not piece_detected or (current_time - last_detection_time >= READING_INTERVAL):
                    send_image_to_server(processed_image_path)
                    piece_detected = True
                    last_detection_time = current_time
                    counter += 1
                else:
                    print("Aguardando intervalo de leitura.")
            else:
